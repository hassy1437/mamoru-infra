...
interface Props {
    ...
    savedPayload?: Partial<WaterSprayBekki4Payload> | null
...
export default function WaterSprayBekki4Form({
    initial,
    soukatsuId,
    itiranId,
    propertyId,
    ...
    const payload = useMemo<WaterSprayBekki4Payload>(() => ({
        form_name: formName,
    ...
    const persistDraft = useCallback(async (clearMessage = true) => {
        ...
        const { error: saveError } = await supabase
            .from("inspection_water_spray_bekki4")
            .upsert({
        ...
        if (saveError) {
            if (saveError.message.includes("inspection_water_spray_bekki4")) {
    ...
    const generatePdfBlob = useCallback(async () => {
        const response = await fetch("/api/generate-water-spray-bekki4-pdf", {
            method: "POST",
...
<div class="msg">PDFを生成しています…</div></body></html>`)
    ...
    const handleDownload = useCallback(async () => {
        ...
        try {
            ...
            a.href = url
            a.download = `水噴霧消火設備点検票_${formName || "bekki4"}.pdf`
    ...
    return (
        <div className="space-y-6">
            <Card>
                <CardHeader>
                    <CardTitle>水噴霧消火設備点検票（別記様式4）入力</CardTitle>
                    <CardDescription>s50_kokuji14_bekki4.pdf に転記する項目です。</CardDescription>
                </CardHeader>
            ...
            {/* 5 = 圧力検知装置の圧力スイッチ。様式が「設定圧力 MPa / 作動圧力 MPa」の2値を求める行 */}
            {renderItemTable("（その3）機器点検・総合点検", PAGE3_ITEMS, page3Rows, setPage3Rows, undefined, undefined, 5)}
