...
const PERIOD_START_ANCHORS: DateAnchors = { year: 299.52, month: 338.05, day: 376.56, baseline: 183.48 }
const PERIOD_END_ANCHORS: DateAnchors = { year: 426.0, month: 464.64, day: 503.04, baseline: 183.48 }
...
export async function POST(req: NextRequest) {
    try {
        ...
        const drawHeader = (page: PDFPage, pageHeight: number) => {
            ...
            drawInCell(page, pageHeight, body.inspector_company, 299.52, 193.2, 103.18, 26.28, 6.0)
        ...
        if (systemOverflow.length) {
            ...
            console.error("[pdf] 収容不能(システム由来)", { form: "別記様式第22", items: systemOverflow })
        }
        const fitFormLabel = "別記様式第22"
        ...
        const fitError = buildFitError("別記様式第22", fonts.fit!)
