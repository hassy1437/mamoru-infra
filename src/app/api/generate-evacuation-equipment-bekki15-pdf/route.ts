...
const PERIOD_START_ANCHORS: DateAnchors = { year: 311.88, month: 343.45, day: 374.9, baseline: 177.36 }
const PERIOD_END_ANCHORS: DateAnchors = { year: 427.46, month: 459.03, day: 490.47, baseline: 177.36 }
...
export async function POST(req: NextRequest) {
    try {
        ...
        const periodErr = periodDateError("別記様式第15", body.period_start, body.period_end)
        ...
        const drawResultRows = (
            ...
            for (let i = 0; i < rowBounds.length - 1; i += 1) {
                ...
                drawWrappedInCell(page, pageHeight, row.content, cols.contentX, top, cols.contentW, h, sizes?.content ?? 6.2, ref("content"))
        ...
        const drawHeader = (page: PDFPage, pageHeight: number) => {
            ...
            // 刷り込みに重ねない: 前置ラベル「氏名」(-143.9) の右から（テンプレート実測）
            drawInCell(page, pageHeight, body.inspector_name, 143.9, 181.2, 62.4, 52.56, 6.7)
            // 刷り込みに重ねない: 「TEL」(406.4-) の手前まで（テンプレート実測）
            drawInCell(page, pageHeight, body.inspector_company, 302.4, 181.2, 104.0, 26.28, 6.4)
