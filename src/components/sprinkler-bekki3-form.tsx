...
interface Props {
    ...
    savedPayload?: Partial<SprinklerBekki3Payload> | null
...
export default function SprinklerBekki3Form({
    initial,
    soukatsuId,
    itiranId,
    propertyId,
    ...
    const [page5Rows, setPage5Rows] = useState<RowState[]>(() => hydrateRows(PAGE5_ITEMS.length, saved.page5_rows))
    ...
    const payload = useMemo<SprinklerBekki3Payload>(() => ({
        form_name: formName,
    ...
    const persistDraft = useCallback(async (clearMessage = true) => {
        ...
        if (saveError) {
            if (saveError.message.includes("inspection_sprinkler_bekki3")) {
    ...
    const generatePdfBlob = useCallback(async () => {
        const response = await fetch("/api/generate-sprinkler-bekki3-pdf", {
            method: "POST",
    ...
    const handleDownload = useCallback(async () => {
        ...
        try {
            ...
            a.href = url
            a.download = `スプリンクラー設備点検票_${formName || "bekki3"}.pdf`
    ...
    return (
        <div className="space-y-6">
            <Card>
                <CardHeader>
                    <CardTitle>スプリンクラー設備点検票（別記様式3）入力</CardTitle>
                    <CardDescription>s50_kokuji14_bekki3.pdf に転記する項目です。</CardDescription>
                </CardHeader>
            ...
            {pdfUrl && (
                <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
                    <iframe src={pdfUrl} title="スプリンクラー設備点検票PDFプレビュー" className="w-full h-[75vh] md:h-[calc(100vh-220px)]" />
                </div>
            )}
        </div>
    )
