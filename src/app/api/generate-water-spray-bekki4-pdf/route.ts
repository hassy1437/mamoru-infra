...
const PERIOD_START_ANCHORS = { year: 323.2, month: 355.0, day: 387.2, baseline: 171.36 }
const PERIOD_END_ANCHORS = { year: 450.8, month: 482.5, day: 514.5, baseline: 171.36 }
...
export async function POST(req: NextRequest) {
    try {
        ...
        const periodErr = periodDateError("別記様式第4", body.period_start, body.period_end)
        ...
        for (const ri of [4, 6] as const) {
            ...
            drawInCellWithFont(page2, p2Height, fonts, pressRow.content, 283, pTop, 14, pH, 6.5, { paddingX: 0.5,
        ...
        if (perfRow4) {
            ...
            const drawFlowVal = (v: string) => drawInCellWithFont(page2, p2Height, fonts, v, 292, perfTop, 15, perfH, 6.5, { paddingX: 1 })
        ...
        // ★skipContentRows は内容列しか止めないので、判定・不良内容・措置内容が見出しの上に描かれていた。
        //   他10ルートと同じ blankPrintedRows（行データごと空にする）に揃える。
        drawResultRows(page3, p3Height, blankPrintedRows(body.page3_rows, new Set([12])), P3_ROW_BOUNDS, {
            rowsKey: "page3_rows",
            contentX: 238, contentW: 105,
        ...
        //   実測の罫線: name 82.80〜83.28 / model 138.60〜139.08 / date 194.52〜195.00
        //               device2.name 306.84〜307.32
        drawInCell(page3, p3Height, device1.name, 82.39, 658, 55.61, 18, 7.2, devOpts)
        ...
        const fitError = buildFitError("別記様式第4", fonts.fit!)
